...
window.onload = function () {
	...
	toggleMirrorBtn.addEventListener('click', function () {
		...
		toggleMirrorBtn.textContent = isMirrorEnabled ? '切换原纹理' : '切换镜面效果';
		render();
	});
...
function render() {
	...
	// 确保立方体贴图也被绑定，用于中间立方体的镜面效果
	gl.activeTexture(gl.TEXTURE2);
	gl.bindTexture(gl.TEXTURE_CUBE_MAP, cubeMap);
	gl.uniform1i(gl.getUniformLocation(program, "cubeSampler"), 2);
	// 传递镜面效果控制变量到shader
	gl.uniform1i(gl.getUniformLocation(program, "isMirrorEnabled"), isMirrorEnabled ? 1 : 0);
	gl.drawArrays(gl.TRIANGLES, 0, cubenumPoints);
