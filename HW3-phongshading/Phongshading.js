...
window.onload = function() {
	...
	if (!program) {
		console.error("Failed to initialize box shader program");
		return;
	}
	...
	lampProgram = initShaders(gl, "shaders/lamp.vert", "shaders/lamp.frag");
	if (!lampProgram) {
		console.error("Failed to initialize lamp shader program");
		return;
	}
	depthProgram = initShaders(gl, "shaders/depth.vert", "shaders/depth.frag");
	if (!depthProgram) {
		console.error("Failed to initialize depth shader program");
		return;
	}
	...
	configurePhongModelMeterialParameters(lampProgram);
	initArrayBuffer(gl, lampProgram, flatten(points), 4, gl.FLOAT, "vPosition");
	//调用绘制函数
	render();
}
...
function render(){	
	...
	// 绘制中间立方体时设置为镜面
	gl.useProgram(program);
	gl.uniform1i(gl.getUniformLocation(program, "isMirror"), 1);
	// 绘制其他物体时设置为0
	gl.uniform1i(gl.getUniformLocation(program, "isMirror"), 0);
...
gl.useProgram(lampProgram);
	gl.uniformMatrix4fv(gl.getUniformLocation(lampProgram,"u_ModelMatrix"), false, flatten(ModelMatrix));
	gl.uniformMatrix4fv( gl.getUniformLocation(lampProgram,"u_ViewMatrix"), false, flatten(ViewMatrix));
	gl.uniformMatrix4fv( gl.getUniformLocation( lampProgram, "u_ProjectionMatrix" ),false, flatten(ProjectionMatrix));	
